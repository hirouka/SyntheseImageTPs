...
void Viewer::drawScene() {
  ...
  //PLANETE 2
    ...
    // eventually, apply some transformationss to the current (top) model matrix
    _modelMatrices.top() = glm::rotate(_modelMatrices.top(),_spin, glm::vec3(0.0f,0.0f,1.0f));   
    _modelMatrices.top() = glm::translate(_modelMatrices.top(), glm::vec3(0.0f,4.0f,0.0f)); 
    _modelMatrices.top() = glm::scale(_modelMatrices.top(), glm::vec3(0.5f,0.5f,0.5f));  
...
void Viewer::paintGL() {
  ...
  //A COMPLETER 
  _viewMatrix = glm::lookAt(glm::vec3(0.0f,0.0f,10.0f),glm::vec3(0.0f,0.0f,0.0f),glm::vec3(0.0f,1.0f,0.0f));
  _projMatrix = glm::perspective(45.0f,(float)(width()/height()),0.1f,100.0f);
  // draw the scene 
  drawScene();
